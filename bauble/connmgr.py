        ...
        if os.path.exists(pics) and os.path.isdir(pics):
            if os.path.exists(thumbs):
                if not os.path.isdir(thumbs):
                    valid.append(False)
                    msg += _("Thumbs directory name occupied by non directory")
            else:
                os.mkdir(thumbs)
    valid = all(valid)
...
class ConnMgrPresenter(GenericEditorPresenter):
    ...
    def __init__(self, view=None):
        ...
        if self.connection_names:
            ...
            self.connection_name = None
        super().__init__(
            model=self, view=view, refresh_view=True, session=False
        )
...
def start_connection_manager(msg=None):
    ...
    tooltips = {
        "rootdir2_entry": _(
            "Set a directory to store file data in.  If left "
