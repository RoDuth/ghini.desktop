...
def get_field_properties(model, path):
    ...
    try:
        ...
        # relationships
        if path == model.__table__.key or hasattr(
            getattr(getattr(model, path), "property"), "back_populates"
        ):
            return "C", MAX_LENGTH
    try:
...
class ShapefileExportSettingsBox(Gtk.ScrolledWindow):
    ...
    def _construct_grid(
        ...
        for row, (name, typ, size, path) in enumerate(self.fields):
            ...
            length_adj = Gtk.Adjustment(
                upper=MAX_LENGTH, step_increment=1, page_increment=10
            )
            length_entry = Gtk.SpinButton(adjustment=length_adj, numeric=True)
            if typ in ["F", "N"]:
    ...
    def generated_points_settings_dialog(self):
        ...
        x_adj = Gtk.Adjustment(
            upper=1000, lower=-1000, step_increment=0.0001, page_increment=10
        )
        gen_x_entry = Gtk.SpinButton(adjustment=x_adj, digits=10)
        gen_x_entry.set_tooltip_text(
            "set the X coordinate for the starting "
        ...
        gen_y_entry.set_tooltip_text(
            "set the Y coordinate for the starting "
            "point. Default is the institution's "
            "latitude."
        )
        ...
        gen_combo.set_tooltip_text(
            "Set the axis for the line of auto-generated plant points."
        )
        for i in ["", "NS", "EW"]:
        ...
        gen_increment_entry = Gtk.SpinButton(
            adjustment=increment_adj, digits=10
        )
    ...
    def reset_gen_settings(self):
        ...
        start_lat = get_default("inst_geo_latitude", session=session).value
        start_lng = get_default("inst_geo_longitude", session=session).value
    ...
    def on_remove_button_dragged(self, widget, drag_context, data, info, time):
        logger.debug(
            "drag event = context: %s data: %s info: %s time: %s",
            drag_context,
            data,
            info,
            time,
    ...
    def on_remove_button_dropped(
        ...
        logger.debug(
            "drop event = context: %s x: %s y: %s data: %s info: %s "
            "time: %s",
            drag_context,
            x,
            y,
    ...
    def on_remove_button_clicked(self, widget):
        ...
        self.grid.remove_row(
            self.grid.child_get_property(widget, "top_attach")
        )
        del self.fields[row]
        self.resize_func()
...
class ShapefileExportDialogPresenter(GenericEditorPresenter):
    ...
    def __init__(self, model, view):
        ...
        if (
            not isinstance(main_view, SearchView)
            or main_view.results_view.get_model() is None
        ):
        ...
        self.add_problem(self.PROBLEM_EMPTY, self.view.widgets.input_dirname)
        self.settings_boxes = []
        self.refresh_view()
    ...
    def _settings_expander(self):
        ...
        if self.model.export_plants:
            ...
            notebook.append_page(plt_settings_box, Gtk.Label(label="Plants"))
        ...
        if self.model.export_locations:
            ...
            notebook.append_page(
                loc_settings_box, Gtk.Label(label="Locations")
            )
...
class ShapefileExporter(GenericExporter):
    ...
    SHAPE_MAP = {"Polygon": "poly", "LineString": "line", "Point": "point"}

    _tooltips = {
        "plants_locations": _(
            "What is it that you want to export.  Plants "
            "may produce multiple files."
        ),
        "search_or_all": _(
            "Base the export of the results on a search or all "
    ...
    def __init__(self, view=None, proj_db=None, open_=True):
        ...
        self.plant_fields = [
            ...
            for k, v in self.plant_fields.items()
        ]
        self.location_fields = [
            [k, *get_field_properties(Location, v), v]
            for k, v in self.location_fields.items()
        ]
    ...
    def run(self):
        ...
        if all(
            (
                bool(v[0]) and bool(v[1]) if isinstance(v, list) else bool(v)
                for k, v in self.gen_settings.items()
            )
        ):
            ...
            with ExitStack() as stack:
                ...
                if self.generated_items:
                    ...
                    for records_done, item in enumerate(self.generated_items):
                        if records_done % five_percent == 0:
                            pb_set_fraction(
                                records_done / len(self.generated_items)
                            )
                            yield
                        self.add_generated_points(
                            item, records_done, fields, shapefiles
                        )
    ...
    def add_generated_points(self, item, records_done, fields, shapefiles):
        ...
        xxx = float(self.gen_settings.get("start")[0]) + (
            increment_x * records_done
        )
        yyy = float(self.gen_settings.get("start")[1]) + (
            increment_y * records_done
        )
        item.geojson = {"type": "Point", "coordinates": [xxx, yyy]}
    ...
    def get_shapes_and_items(self, session, model, allowable_shapetypes):
        if self.search_or_all == "rb_search_results":
            ...
            export_items = (
                session.query(model).filter(model.geojson.isnot(None)).all()
            )
    ...
    def create_prj_file(self, shapefilename):
        ...
        if not sys_proj_str:
            ...
            msg = _(
                "Cannot proceed without a system CRS. To set a system "
        ...
        if not self.proj_db.get_prj(sys_proj_str):
            ...
            msg = _(
                "Cannot proceed without a .prj file string for the system "
    ...
    def add_field(shapefile, field_name, field_type, field_size=None):
        if field_size and field_type in ["N", "F"]:
            shapefile.field(field_name, field_type, decimal=field_size)
        elif field_size and field_type == "C":
            shapefile.field(field_name, field_type, size=field_size)
        else:
        ...
        try:
            ...
            if self._generate_points > 0:
                if self.error > 400:
                    ...
                    msg = (
                        "<b>Over 400 records have no geojson.\n\n This is "
