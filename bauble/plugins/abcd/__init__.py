...
def verify_institution(inst):
    ...
    return (
        verify(inst.name)
        and verify(inst.technical_contact)
        and verify(inst.email)
        and verify(inst.contact)
        and verify(inst.code)
    )
        ...
        if for_reports:
            ...
            for note in notes_list:
                etree.SubElement(
                    note_unit,
                    note["category_name"],
                    attrib={"User": note["user"], "Date": note["date"]},
                ).text = note["text"]
...
class AccessionABCDAdapter(SpeciesABCDAdapter):
    ...
    def extra_elements(self, unit):
        ...
        if self.accession.source and self.accession.source.collection:
            ...
            if collection.notes:
                abcd_element(
                    gathering, "Notes", utils.xml_safe(collection.notes)
                )
...
class ABCDCreator:
    def __init__(
        self, decorated_objects: Iterable[ABCDAdapter], authors: bool = True
    ) -> None:
        """
    ...
    def _create_units_element(self) -> SubElement:
        ...
        if not verify_institution(self.inst):
            msg = _(
                "Some or all of the information about your institution or "
    ...
    def generate_elements(self) -> Generator[None, None, None]:
        ...
        for obj in self.decorated_objects:
            ...
            abcd_element(
                botanical, "FirstEpithet", text=obj.get_firstepithet()
            )
            if obj.get_infraspecificepithet() and obj.get_infraspecificrank():
                ...
                abcd_element(
                    botanical, "Rank", text=obj.get_infraspecificrank()
                )
            ...
            if obj.get_cultivarname():
                abcd_element(
                    botanical, "CultivarName", text=obj.get_cultivarname()
                )
            ...
            if notes_list:
                for note in notes_list:
                    ...
                    notes_str += (
                        f'{note["category"]} = {note["text"]} '
                        f'({note["user"]} : {note["date"]})|'
                    )
                abcd_element(
                    unit, "Notes", text=utils.xml_safe(str(notes_str))
                )
            yield
