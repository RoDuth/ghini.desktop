...
class GenusEditorView(editor.GenericEditorView):
    _tooltips = {
        ...
        "gen_hybrid_combo": _(
            'Genus hybrid flag, a named hybrid ("x") or a '
            'graft chimaera ("+")'
        ),
    ...
    def __init__(self, parent=None):
        ...
        super().__init__(
            filename, parent=parent, root_widget_name="genus_dialog"
        )
...
class GenusEditorPresenter(editor.GenericEditorPresenter):
    ...
    def __init__(self, model, view):
        ...
        def fam_get_completions(text):
            ...
            return query.filter(
                utils.ilike(Family.family, f"{text}%%")
        ...
        def on_select(value):
            ...
            def on_response(_button, response):
                ...
                if response:
                    ...
                    self.view.widgets.gen_family_entry.set_text(
                        utils.nstr(syn.family)
                    )
        ...
        self.assign_completions_handler(
            "subfamily_entry", self.subfam_get_completions, set_problems=False
        )
        self.assign_completions_handler(
            "tribe_entry", self.tribe_get_completions, set_problems=False
        )
        self.assign_completions_handler(
            "subtribe_entry", self.subtribe_get_completions, set_problems=False
        )
        self.assign_simple_handler(
            "gen_hybrid_combo", "hybrid", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "gen_qualifier_combo", "qualifier", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "gen_genus_entry", "genus", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "gen_author_entry", "author", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
        ...
        self.refresh_cites_label()
        self.assign_simple_handler(
            "subfamily_entry", "subfamily", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "tribe_entry", "tribe", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "subtribe_entry", "subtribe", editor.StringOrNoneValidator()
        )
    ...
    def tribe_get_completions(self, text):
        ...
        query = query.filter(utils.ilike(Genus.tribe, f"{text}%%")).distinct()
        return [i[0] for i in query]
