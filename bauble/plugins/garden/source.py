...
collection_add_plant_action = Action(
    "collection_add",
    _("_Add plants"),
...
collection_remove_action = Action(
    "collection_remove",
    _("_Delete"),
...
class Collection(db.Domain):
    ...
    region = relationship(
        Geography, uselist=False, backref=backref("collection", uselist=True)
    )
    ...
    def retrieve(cls, session, keys):
        ...
        if acc_key:
            ...
            if not retrieved_acc:
                return None
            query = query.join(Source, Accession).filter(
                Accession.id == retrieved_acc.id
            )
...
class CollectionPresenter(editor.ChildPresenter):
    ...
    def __init__(self, parent, model, view, session):
        ...
        self.assign_simple_handler(
            "locale_entry", "locale", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "collid_entry", "collectors_code", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
        ...
        self.assign_simple_handler(
            "habitat_textview", "habitat", editor.StringOrNoneValidator()
        )
        self.assign_simple_handler(
            "coll_notes_textview", "notes", editor.StringOrNoneValidator()
        )
        ...
        self.assign_simple_handler(
            "datum_entry", "gps_datum", editor.StringOrNoneValidator()
        )
    ...
    def on_lat_entry_changed(self, entry):
        ...
        try:
            ...
            ArithmeticError,
            ValueError,
            bauble.error.CheckConditionError,
        ) as e:
            logger.debug("%s(%s)", type(e).__name__, e)
            self.add_problem(
...
class PropagationChooserPresenter(editor.ChildPresenter):
    ...
    def __init__(self, parent, model, view, session=None):
        ...
        self.view.connect_after(
            prop_toggle_cell, "toggled", self.on_prop_toggle_cell_toggled
        )
    ...
    def on_prop_toggle_cell_toggled(self, cell, path):
        ...
        if not cell.get_active():  # it's not active make it active
            ...
            acc_view.widget_set_value(
                "acc_species_entry", utils.nstr(prop.plant.accession.species)
            )
            acc_view.widget_set_value(
                "acc_id_qual_combo", utils.nstr(prop.plant.accession.id_qual)
            )
...
source_detail_edit_action = Action(
    "source_detail_edit",
    _("_Edit"),
...
source_detail_remove_action = Action(
...
source_detail_add_acc_action = Action(
    "source_detail_add_acc",
    _("_Add accession"),
...
class SourceDetail(db.Domain):
    ...
    source_type = Column(
        types.Enum(
            values=list(_source_types.keys()), translations=_source_types
        ),
        default=None,
    )
    ...
    def count_children(self):
        session = object_session(self)
        query = session.query(Source.id).filter(
            Source.source_detail_id == self.id
        )
        if prefs.prefs.get(prefs.exclude_inactive_pref):
            cls = Source.accession.prop.mapper.class_
...
class SourceDetailPresenter(editor.GenericEditorPresenter):
    ...
    def __init__(
        self, model, view, do_commit=True, source_types=None, **kwargs
    ):
        ...
        view.init_translatable_combo(source_type_combo, _source_type_vals)

        super().__init__(
            model, view, refresh_view=True, do_commit=do_commit, **kwargs
        )

        # set combo default after super().__init__
        if default_source_type:
            treeiter = utils.combo_get_value_iter(
                source_type_combo, default_source_type
            )
